...
try:
    ...
    raise ModuleNotFoundError('\nPlease run "pip install retro[atari]"')
...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (4, 320, 224)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = retro.make("Airstriker-Genesis", state='Level1')
        if seed is not None:
            self.env.seed(seed)
        self.env = AirstrikerDiscretizer(self.env)			# 行動空間を離散空間に変換
        self.env = CustomRewardAndDoneEnv(self.env)			# 報酬とエピソード完了の変更
        self.env = StochasticFrameSkip(self.env, n=4, stickprob=0.25)	# スティッキーフレームスキップ
        self.env = Downsample(self.env, 2)				# ダウンサンプリング
        self.env = Rgb2gray(self.env)				# グレースケール
        self.env = FrameStack(self.env, 4)				# フレームスタック
        self.env = ScaledFloatFrame(self.env)				# 状態の正規化
    ...
    def step(self, action):
        ...
        observation = cv2.resize(observation, (224, 320), interpolation=cv2.INTER_AREA)
